...
I18N = {
    "Chinese": {
        ...
        "placeholder_extra": "请详细描述您的背景，让 AI 更有针对性地筛选...",
        "btn_run": "🚀 开始 AI 全权决策匹配",
        "report_title": "✨ AI 深度决策分析报告",
        "original_data": "查看 AI 扫描的原始数据池",
        "db_title": "📋 奖学金数据库全清单",
        "search_db": "🔍 在数据库中搜索...",
        "no_db": "⚠️ 未发现数据库文件，请检查系统。",
        "scanning": "🤖 正在全库扫描并决策中..."
    },
    "English": {
        ...
        "lbl_extra": "Detailed Background (GPA, Experience, etc.)",
        "placeholder_extra": "Describe your background in detail for better AI matching...",
        ...
        "search_db": "🔍 Search in database...",
        "no_db": "⚠️ Database file not found.",
        "scanning": "🤖 Scanning & analyzing data..."
    }
}
            ...
            with result_container.container():
                if df.empty:
                    ...
                    with st.spinner(texts["scanning"]):
                        # 简单的相关性初步筛选（给 AI 提供最相关的 20 条，防止超长）
                        ...
                        system_prompt = f"""
                        You are a professional UWA Scholarship Expert. Respond in {target_lang}.
                        
                        STYLE RULES:
                        1. NO [BRACKETS] for headers. Use bold text and Emojis.
                        2. FULL POWER: Read the study level, residency, and GPA requirements carefully. You decide the match.
                        3. BE DIRECT: If the user doesn't fit, say it clearly with ⚠️.
                        
                        STRUCTURE:
                        - 🏆 **最推荐的项目 / Top Recommendations**
                        - 🔍 **匹配度深度测评 / Deep Eligibility Check**
                        - 💡 **后续行动建议 / Action Plan**
                        """
                        ...
                        try:
                            response = client.chat.completions.create(
                                ...
                                messages=[
                                    ...
                                    {"role": "user", "content": f"User Level: {level}, Major: {major}, Intl: {is_intl}\nDetails: {user_query}\n\nCandidate Pool:\n{all_data_text}"}
                                ]
                            )
